//Program to search for a value in BinarySeachTree
//CC
...
public class W8CC5 { 
    ...
    static Node search_Recursive(Node root, int key) { 
        if (root == null || root.key == key) // when root is null or key is present at root
        return root; 
        if (root.key > key) 
        return search_Recursive(root.left, key); // when value is greater than root's key
        return search_Recursive(root.right, key); // when value is less than root's key
    }
